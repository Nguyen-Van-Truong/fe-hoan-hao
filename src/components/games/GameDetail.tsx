...
import { GAMES_DATA, GameStatus } from "@/data/games";
...
const GameDetail = () => {
  ...
  if (game.status === "playable") {
    switch (game.title) {
      ...
      // case "Đoán Từ":
      //   return <WordGuessGamePage gameId={gameId} />;
      // case "Xếp Hình":
      //   return <PuzzleGamePage gameId={gameId} />;
      default:
        // For playable games that haven't been implemented yet
        return renderGamePlaceholder(game);
    }
  } else {
    // For non-playable games
    return renderGamePlaceholder(game);
  }
};

// Helper function to render a placeholder for games that are not yet implemented or not playable
const renderGamePlaceholder = (game: any) => {
  return (
